...
* On the server switch by using the ```su ``` command to the grader user and make sure you're in the home directory ```cd ~ ```
...
```
$ ssh grader@instanceIP -p 2200 -i ~/.ssh/keys-file
```
...
```
$ git clone https://github.com/MohamedAbdelalim/catalog.git
```
